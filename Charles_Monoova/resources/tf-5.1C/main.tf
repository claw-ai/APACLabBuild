...
# Dev vDBs
## CRM Dev vDB
resource "delphix_vdb" "5.1C" {
    depends_on              = [ delphix_vdb.5.1B ]
    name                    = "5.1B"
    source_data_id          = delphix_vdb.5.1B.id
    ...
    appdata_source_params = jsonencode({
        mountLocation       = "/mnt/provision/monoova/5.1C"
        postgresPort        = 8062
    })
...
resource "delphix_vdb_group" "customer" {
    depends_on      = [ delphix_vdb.5.1C ]
    name            = "customer"
    vdb_ids         = [ delphix_vdb.5.1C.id ]
